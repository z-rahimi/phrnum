...
protected:
	...
	//my code
	typedef std::map<std::string, Scores > _ScoreCacheTable;
 	//_ScoreCacheTable m_cachedBWCScores;
public:
	std::string m_cPhrase;
	//
...
public:
	...
	/** returns cached scores */
	/*inline const Scores *GetBWCCachedScores(const std::string key) const
	{
		_ScoreCacheTable::const_iterator it = m_cachedBWCScores.find(key);
		if(it == m_cachedBWCScores.end()){
		  VERBOSE(3, "NULL\n");
		  return NULL;
		}
		else
	...
	//my code
	//void CacheBWCScores(std::map< std::string, Moses::Scores >& m);
	void CacheClassString(std::string& str);
	//
