...
LexicalReorderingConfiguration::LexicalReorderingConfiguration(ScoreProducer *scoreProducer, const std::string &modelType)
  ...
  for (size_t i=0; i<config.size(); ++i) {
    if (config[i] == "hier") {
      m_phraseBased = false;
    } else if (config[i] == "phrase" || config[i] == "phrnumlex" || config[i] == "BoundaryWordClass") {
      m_phraseBased = true;
...
PhraseBasedReorderingState::PhraseBasedReorderingState(const PhraseBasedReorderingState *prev, const TranslationOption &topt)
  : LexicalReorderingState(prev, topt), m_prevRange(topt.GetSourceWordsRange()), m_first(false), m_prevClassStr((topt.m_cPhrase)) {}
...
//my code: BWC
 /*LexicalReorderingState* PhraseBasedReorderingState::BWCExpand(const TranslationOption& topt, Scores& scores) const{
   VERBOSE(3, "PhraseBasedReorderingState::BWCExpand\n");
   ReorderingType reoType;
  const WordsRange currWordsRange = topt.GetSourceWordsRange();
  const LexicalReorderingConfiguration::ModelType modelType = m_configuration.GetModelType();
   ...
   if (!m_first) {
    ...
    CopyBWCScores(scores,topt,reoType);
  }
  return new PhraseBasedReorderingState(this, topt);
}

void PhraseBasedReorderingState::CopyBWCScores(Scores& scores, const Moses::TranslationOption& topt, LexicalReorderingState::ReorderingType reoType) const
{
  // don't call this on a bidirectional object
  VERBOSE(3, "PhraseBasedReorderingState::CopyBWCScores\n");
  assert(m_direction == LexicalReorderingConfiguration::Backward);
  const Scores *cachedScores = topt.GetBWCCachedScores(m_prevClassStr);
...
//my code: Coil
LexicalReorderingState* PhraseBasedReorderingState::CoilExpand(const TranslationOption& topt, Scores& scores, int segNum) const{
  VERBOSE(3, "PhraseBasedReorderingState::CoilExpand\n");
  ReorderingType reoType;
  const WordsRange currWordsRange = topt.GetSourceWordsRange();
  const LexicalReorderingConfiguration::ModelType modelType = m_configuration.GetModelType();
  ...
  /*if (m_direction == LexicalReorderingConfiguration::Forward && m_first) {
      ClearScores(scores);
  } if (m_direction == LexicalReorderingConfiguration::Backward) {
    if (modelType == LexicalReorderingConfiguration::MSD) {
      reoType = GetOrientationTypeMSD(currWordsRange);
    } else if (modelType == LexicalReorderingConfiguration::MSLR) {
      reoType = GetOrientationTypeMSLR(currWordsRange);
    ...
    int delta = calStep(currWordsRange);
    CopyCoilScores(scores, topt, reoType, segNum, delta);
  }*/
  int delta = calStep(currWordsRange);
  CopyCoilScores(scores, topt, reoType, segNum, delta);
  return new PhraseBasedReorderingState(this, topt);
  VERBOSE(3, "exit from PhraseBasedReorderingState::CoilExpand\n");
  
}

int PhraseBasedReorderingState::calStep(WordsRange currRange) const {
  //VERBOSE(3,"OI-> calStep1: " << "\n");
  int delta= (int)currRange.GetStartPos() - (int)m_prevRange.GetEndPos();
  return delta;
}

void PhraseBasedReorderingState::CopyCoilScores(Moses::Scores& scores, const Moses::TranslationOption& topt, Moses::LexicalReorderingState::ReorderingType reoType, int segNum, int delta) const
{
  VERBOSE(3, "PhraseBasedReorderingState::CopyCoilScores\n");
  float lexScore, phrNumScore = 0;
//   assert(m_direction == LexicalReorderingConfiguration::Backward || m_direction == LexicalReorderingConfiguration::Forward);
...
//   CopyScores(scores, topt, reoType);
  float alpha = 0.1;
  std::map<int, std::map<int, std::vector<float> > >::const_iterator it1 = m_configuration.m_nonParaPhrNum.find(segNum);
  VERBOSE(3, "seg+delta: " << segNum << " , " << delta  << "\n");
  std::map<int, std::vector<float> > deltaScore;
//   if(m_direction == LexicalReorderingConfiguration::Backward){
    if(it1 != m_configuration.m_nonParaPhrNum.end()){
//       VERBOSE(3,"khodaaaaa\n");
      deltaScore = (*it1).second;
      std::map<int, std::vector<float> >::const_iterator it = deltaScore.find(abs(delta));
      if(it != deltaScore.end()){
	const Scores &scoreSet = (*it).second;
// 	VERBOSE(3, "scoreSet[flag]: " << scoreSet[flag]  << " ,scoreSet[flag]*alpha: " << scoreSet[flag]*(1-alpha) << "\n");
	phrNumScore = scoreSet[flag]*(1-alpha);
      }
    }
    VERBOSE(3, "phrNumScore1: " << phrNumScore  << "\n");
    
    float av, va;
    std::map<int, std::vector<float> > average, variance;
    average = m_configuration.m_average;
    variance = m_configuration.m_variance;
    
    if(segNum < variance.size()){
      av = (average[segNum])[flag];
      va = (variance[segNum])[flag];
    }
    if(segNum >= variance.size() || av == 0){
      int index = variance.size();
      av = (average[index])[flag];
      va = (variance[index])[flag];
    }
    
    phrNumScore += alpha*poDist(av, delta);
    VERBOSE(3, "phrNumScore2: " << alpha*poDist(av, delta)  << "\n");
    if(m_configuration.CollapseScores())
      scores[m_offset] += TransformScore(phrNumScore);
    else {
      scores[/*scores.size() - 2 + */flag] += TransformScore(phrNumScore);
    }
  //}
  //VERBOSE(3, "scores2: " <<  scores[flag] << endl);
}

float LexicalReorderingState::poDist(float mean, int step) const{
  float result=0;
  result = (exp(-abs(mean))*pow(mean,abs(step)))/fact(abs(step));
  return result;
}

int LexicalReorderingState::fact(int num) const{
  int res = 1;
  for(int i = 1;i <=num; i++){
    res *= i; 
  }
...
//my code:Coil
/* LexicalReorderingState* BidirectionalReorderingState::CoilExpand(const TranslationOption& topt, Scores& scores, int segNum) const{
   VERBOSE(3, "BidirectionalReorderingState::CoilExpand 1\n");
   LexicalReorderingState *newbwd = m_backward->CoilExpand(topt, scores, segNum);
   LexicalReorderingState *newfwd = m_forward->CoilExpand(topt, scores, segNum);
