...
class LexicalReorderingTable {
 ...
 LexicalReorderingTable(const FactorList& f_factors, const FactorList& e_factors, const FactorList& c_factors)
  ...
  void LoadNonParaPhrNum(const std::string& filePath);
  void LoadParaPhrNum(const std::string& filePath);
  
  virtual ~LexicalReorderingTable(){
  }
 public:
  public:
  static LexicalReorderingTable* LoadAvailable(const std::string& filePath, const std::string& nonParaPhrNumfilePath, const std::string& paraPhrNumfilePath, const Moses::FactorList& f_factors, const Moses::FactorList& e_factors, const Moses::FactorList& c_factors, const std::string& flag);

 public:
  virtual Scores GetScore(const Phrase& f, const Phrase& e, const Phrase& c) = 0;
  //my code
  //virtual std::string GetWClassScore(const std::string& key, const std::string& flag){};
  //virtual std::vector<std::string> GetBWCScore(const std::string& key){}
  //
 ...
 protected:
  ...
  typedef  std::map<int, std::vector<float> > ParaPhrNum;
  ParaPhrNum m_average, m_variance;
  //end
...
class LexicalReorderingTableMemory : public LexicalReorderingTable {
 ...
 public:
  ...
  LexicalReorderingTableMemory( const std::string& filePath,
				...
				const std::vector<FactorType>& c_factors,
				std::string flag);
  LexicalReorderingTableMemory(const std::string& filePath1, const std::string& sClassPath, const std::string& tClassPath);
 ...
 private:
  ...
  //my code
  void LoadAllModels(const std::string& filePath);
  //void LoadWCBoundary(const std::string& filePath);
  //void loadWordClasses(const std::string& filePath, std::map< std::string, std::string >& out);
  //virtual std::string GetWClassScore(const std::string& key, const std::string& flag);
 // virtual std::vector<std::string> GetBWCScore(const std::string& key);
  //
 ...
 private:
  ...
  typedef std::map< std::string, std::vector<std::string> > WCBbasedTable;
  typedef std::map< std::string, std::string> WClassTable;
  TableType m_Table; 
  //WCBbasedTable m_WCBtable;
  //WClassTable m_sClasses, m_tClasses;
};
