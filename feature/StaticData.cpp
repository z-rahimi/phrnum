...
bool StaticData::LoadData(Parameter *parameter)
{
  ...
  if (m_parameter->isParamSpecified("translation-details"))
  {
	...
	// score weights
	m_weightWordPenalty				 = 0/*= Scan<float>( m_parameter->GetParam("weight-w")[0] )*/;
  ...
  m_lmcache_cleanup_threshold = (m_parameter->GetParam("clean-lm-cache").size() > 0) ?
	...
	if (!LoadGlobalLexicalModel()) return false;
        if (!LoadPhrNumLexModel()) return false;	
	//if (!LoadBWClassesModel()) return false;
...
//my code: PhrNumLex
//my code
bool StaticData::LoadPhrNumLexModel()
{
    VERBOSE(1, "Loading PhrNumLex distortion models version ..."<< "\n");
    const vector<string> fileStr    = m_parameter->GetParam("phrNumLex-file");
    const vector<float> weights = Scan<float>(m_parameter->GetParam("weight-pl"));
    VERBOSE(1, "have " << fileStr.size() << " models" << std::endl);
   for(int k=0;k<fileStr.size();k++){
     //load all models
       vector<string> specs = Tokenize<string>(fileStr[k]," ");
	
	 // spec[0] = factor map
        // spec[1] = name
        // spec[2] = num weights
        // spec[3] = LexfileName
	// spec[4] = phrNumfileName
	
	// decode factor map
	
	vector<FactorType> input, output;
        vector<string> inputfactors = Tokenize(specs[0],"-");
        if(inputfactors.size() == 2){
            input  = Tokenize<FactorType>(inputfactors[0],",");
            output = Tokenize<FactorType>(inputfactors[1],",");
        } else if(inputfactors.size() == 1) {
	...
	m_modelType = specs[1];;
	size_t numWeights = atoi(specs[2].c_str());
 	string lexfilePath = specs[3];
// 	string nonParaPhrNumfilePath = specs[3];
// 	string paraPhrNumfilePath = specs[4];
	
	if (weights.size() == numWeights){ // check if feature is used
	  m_phrnumlex= new PhrNumLex(input, output, lexfilePath,/* lexfilePath1, nonParaPhrNumfilePath, paraPhrNumfilePath,*/ m_modelType, weights); // create the feature
	}else
	  return false;
...
/*bool StaticData::LoadBWClassesModel()
{
    VERBOSE(1, "Loading Boundary Word Classes distortion models version ..."<< "\n");
    const vector<string> fileStr    = m_parameter->GetParam("BoundaryWordClass-file");
    const vector<float> weights = Scan<float>(m_parameter->GetParam("weight-bwc"));
    const vector<string> wordClassFileStr    = m_parameter->GetParam("WordClass-file");
    VERBOSE(1, "have " << fileStr.size() << " models" << std::endl);
       for(int k=0;k<fileStr.size();k++){
     //load all models
       vector<string> specs = Tokenize<string>(fileStr[k]," ");
	...
	if (weights.size() == numWeights){ // check if feature is used
	  m_bwclass= new BWClasses(input, output, lexfilePath, wordClassFileStr.at(0), wordClassFileStr.at(1), m_modelType, weights); // create the feature
	}else
	  return false;
