...
// my code
LexicalReorderingTable* LexicalReorderingTable::LoadAvailable(const std::string& filePath1, const std::string& filePath2, const std::string& filePath3, const Moses::FactorList& f_factors, const Moses::FactorList& e_factors, const Moses::FactorList& c_factors, const std::string& flag){
	//decide use Tree or Memory table
	if(FileExists(filePath1 +".binlexr.idx")){
	  //there exists a binary version use that
	  return new LexicalReorderingTableTree(filePath1, f_factors, e_factors, c_factors);
	} else {
	  //use plain memory
	  if(flag == "phrnumlex"){
	   return new LexicalReorderingTableMemory(filePath1, f_factors, e_factors, c_factors, flag);
	  }/*else if(flag == "BoundaryWordClass"){
	    return new LexicalReorderingTableMemory(filePath1, filePath2, filePath3);
	  }*/
	  else
	    return new LexicalReorderingTableMemory(filePath1, f_factors, e_factors, c_factors);
	}
  }

  
  void LexicalReorderingTable::LoadNonParaPhrNum(const std::string& filePath)
{
  ...
  while(!getline(file, line).eof()){
    ...
    int t = 0, phrase ; 
    phrase = atoi(tokens.at(0).c_str());
    std::vector<std::string> parts = Tokenize(tokens.at(1), " , ");
    for(int i = 0; i < parts.size(); i++){
      std::vector<std::string> temp = Tokenize(parts.at(i), ":");
      int delta = atoi(temp.at(0).c_str()); 
      //last token are the probs
      std::vector<float> p = Tokenize<float>(temp.at(1), "*");
//       std::transform(p.begin(),p.end(),p.begin(),TransformScore);
//       std::transform(p.begin(),p.end(),p.begin(),FloorScore);
      (m_nonParaPhrNum[phrase])[delta] = p;
      //VERBOSE(3, "probability: " << p.at(0) << " ," << p.at(1) << "\n");
    }
  }
  
}

void LexicalReorderingTable::LoadParaPhrNum(const std::string& filePath)
{
  VERBOSE(3, "pathhhhhhhhhhhhhhhhhh: " << filePath << "\n" );
  InputFileStream file(filePath);
  std::string line("");
  std::string flag;
  std::vector<std::string> tokens;
  while(!getline(file, line).eof()){
    if(strcmp(Trim(line).c_str(), "average") == 0){
       flag = "average";
    }
     
    else if(strcmp(Trim(line).c_str(), "variance") == 0)
      flag = "variance";
    else
    {
      VERBOSE(3, "flaaaaaaaag: " << flag << "\n");
      tokens = Tokenize(line, " ");
      for(int i = 0; i < tokens.size(); i++){
	std::vector<float> p = Tokenize<float>(tokens.at(i), "*");
	if(flag == "average"){
	  m_average[i+1] = p;
	}
	else if(flag == "variance"){
	  m_variance[i+1] = p;
	}
      }
    }
  }
  VERBOSE(3, "sizeeeeeeeeeeeeeeeeeeeeee: " << m_variance.size() << "\n" );
}
// end
...
LexicalReorderingTableMemory::LexicalReorderingTableMemory( 
				const std::string& filePath,/*
				const std::string& nonParaPhrNumfilePath,
				const std::string& paraPhrNumfilePath,*/
				const std::vector<FactorType>& f_factors, 
				const std::vector<FactorType>& e_factors,
				const std::vector<FactorType>& c_factors,
				std::string flag)
  : LexicalReorderingTable(f_factors, e_factors, c_factors) 
{
      LoadAllModels(filePath);
}

/*LexicalReorderingTableMemory::LexicalReorderingTableMemory(const std::string& filePath1, const std::string& sClassPath, const std::string& tClassPath): LexicalReorderingTable()
{
      LoadWCBoundary(filePath1);
      loadWordClasses(sClassPath, m_sClasses);
      loadWordClasses(tClassPath, m_tClasses);
}*/
...
/*std::vector<std::string> LexicalReorderingTableMemory::GetBWCScore(const std::string& key)
{
    if(m_WCBtable.count(key)){
      return m_WCBtable[key];
    } 
    return std::vector<std::string>();
}


std::string LexicalReorderingTableMemory::GetWClassScore(const std::string& key, const std::string& flag)
{
//   VERBOSE(3, "GetWClassScore\n");
  if(flag == "s"){
    if(m_sClasses.count(key) == 1)
      return m_sClasses[key];
    else
      return "-1";
  }
  else if(flag == "t"){
    if(m_tClasses.count(key))
      return m_tClasses[key];
    else
      return "-1";
  }
  return "-1";
...
//my code
void LexicalReorderingTableMemory::LoadAllModels(const std::string& filePath)
{
  int delimiter = 0;
  std::string flag;
  std::vector<std::string> tokens;
  ...
  while(!getline(file, line).eof()){
    if(strcmp(Trim(line).c_str(), "para") == 0){
      delimiter = 1;
    }
    else if(strcmp(Trim(line).c_str(), "non-para") == 0){
      delimiter = 2;
    }
    else if(delimiter == 1){
      if(strcmp(Trim(line).c_str(), "average") == 0){
  ...
  VERBOSE(3, "sizes: " << m_average.size() << " ," << m_nonParaPhrNum.size() << "\n");
}

/*void LexicalReorderingTableMemory::LoadWCBoundary(const std::string& filePath)
{
  std::string fileName = filePath;
  if(!FileExists(fileName) && FileExists(fileName+".gz")){
	fileName += ".gz";
  ...
  int numScores = -1;
  std::cerr << "Loading WCBtable into memory...";
  
  while(!getline(file, line).eof()){
    std::vector<std::string> tokens = TokenizeMultiCharSeparator(line, "|||");
    key = auxClearString(tokens.at(1));
    val = auxClearString(tokens.at(0)) + "|||" + auxClearString(tokens.at(2));
    if(m_WCBtable.count(key)){
      m_WCBtable[key].push_back(val);
    }
    else{
  ...
  std::cerr << "done.\n  The size of this model is: " <<  m_WCBtable.size() << "\n";
}

void LexicalReorderingTableMemory::loadWordClasses(const std::string& filePath, std::map< std::string, std::string>& out)
{
  std::string fileName = filePath;
  if(!FileExists(fileName) && FileExists(fileName+".gz")){
	fileName += ".gz";
  ...
  std::string line(""), key("");
  int numScores = -1;
  std::cerr << "Loading word classes into memory..." << fileName << "\n";
  while(!getline(file, line).eof()){
    std::vector<std::string> tokens = TokenizeMultiCharSeparator(line, "	");
//     VERBOSE(3,"<" << Trim(tokens.at(0)) << ">ge" );
    out[Trim(tokens.at(0))] = Trim(tokens.at(1));
  }
  std::cerr << "done." << out.size() << "\n";
}
//end
