...
//my code
/*void TranslationOptionCollection::CacheWCBReordering()
{
  const BWClasses *obj = (StaticData::Instance().GetBWClasstModel());
  if(obj){
      size_t size = m_source.GetSize();
      for (size_t startPos = 0 ; startPos < size ; startPos++){
	  size_t maxSize =  size - startPos;
	  ...
	  for (size_t endPos = startPos ; endPos < startPos + maxSize; endPos++){
		...
		for(iterTransOpt = transOptList.begin() ; iterTransOpt != transOptList.end() ; ++iterTransOpt){
		    ...
		    string source = sourcePhrase->ToString();
		    std::vector<std::string> vec = boundaryWords(source);
		    std::string sourceClass = "";
		    for(int i = 0; i < vec.size(); i++){
		      if( obj->GetWordClass(Trim(vec.at(i)), "s") == "-1"){
			std::cerr << "source was not found:" << vec.at(i) << endl;
    // 		      exit(-1);
		      }
		      sourceClass += obj->GetWordClass(Trim(vec.at(i)), "s") + " ";
		    }
		    sourceClass = Trim(sourceClass);
		    
		    const TargetPhrase targetPhrase = transOpt.GetTargetPhrase();
		    string target = Trim((Tokenize(targetPhrase.ToString(), "::")).at(0));
		    vec = boundaryWords(target);
		    std::string targetClass = "";
		    for(int i = 0; i < vec.size(); i++){
		      if( obj->GetWordClass(vec.at(i), "t") == "-1"){
			  std::cerr << "target was not found: " << vec.at(i) << endl;
    // 		      exit(-1);
		      }
		      targetClass += obj->GetWordClass(Trim(vec.at(i)), "t") + " ";
		    }
		    targetClass = Trim(targetClass);
		    std::string classPhrase = sourceClass + "*" + targetClass;
		    transOpt.CacheClassString(classPhrase);
		    std::vector<std::string> score = obj->GetBWClass(classPhrase);
		    if(!score.empty()){
		      vector<string> temp;
		      map<string, Scores> mainTable;
		      for(int i = 0; i < score.size(); i++){
			temp = Tokenize(score.at(i), "|||");
			std::vector<float> p = Tokenize<float>(Trim(temp.at(1)), " ");
			std::transform(p.begin(),p.end(),p.begin(),TransformScore);
			std::transform(p.begin(),p.end(),p.begin(),FloorScore);
			mainTable[Trim(temp.at(0))] = p;
		      }
		      transOpt.CacheBWCScores(mainTable);
		    }
		}
...
std::vector<std::string> TranslationOptionCollection::boundaryWords(string phrase)
{
  std::vector<std::string> words;
  std::vector<std::string> vec = Tokenize(phrase, " ");
  if(vec.size() >= 2){
    words.push_back(vec.at(0));
    words.push_back(vec.at(vec.size()-1));
  }
  else
    words.push_back(vec.at(0));
  return words;
